...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f'{r} {info} by: {self.client_address}')
        f = BytesIO()
        ...
        doc += '<hr>\n'
        if r:
            doc += '<strong>Success:</strong>'
        else:
            doc += '<strong>Failed:</strong>'
        doc += info
        doc += f"<br><hr><a href=\"{self.headers['referer']}\">back</a>"
        doc +='"</body>\n</html>\n'
        f.write(str.encode(doc))
        length = f.tell()
        f.seek(0)
    ...
    def deal_post_data(self):
        ...
        line = self.rfile.readline()
        remainbytes -= len(line)
        fn = re.findall(r'Content-Disposition.*name="file"; filename="(.*)"', bytes.decode(line))
        if not fn:
        ...
        while remainbytes > 0:
            line = self.rfile.readline()
            remainbytes -= len(line)
            if str.encode(boundary) in line:
                preline = preline[0:-1]
                if preline.endswith(b'\r'):
                    preline = preline[0:-1]
                out.write(preline)
                out.close()
                return (True, f"File '{os.path.basename(fn)}' upload success!")
            else:
                out.write(preline)
                preline = line
    ...
    def list_directory(self, path):
        ...
        doc += '<hr>\n'
        doc += '<form ENCTYPE=\"multipart/form-data\" method=\"post\">'
        doc += '<input name=\"file\" type=\"file\"/>'
        doc += '<input type=\"submit\" value=\"upload\"/></form>\n'
        doc += '<hr>\n<ul>\n'
        for name in list:
            ...
            doc += f'<li><a href="{urllib.parse.quote(linkname)}">{html.escape(displayname)}</a>\n'
        doc += '</ul>\n<hr>\n</body>\n</html>\n'
        f.write(str.encode(doc))
        length = f.tell()
        f.seek(0)
